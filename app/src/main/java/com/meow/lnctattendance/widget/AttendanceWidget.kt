...
class AttendanceWidget : GlanceAppWidget() {
    ...
    override suspend fun provideGlance(context: Context, id: GlanceId) {
        ...
        val shouldFetch = isInternetAvailable(context) &&
                          (now - lastFetchMs) >= THREE_HOURS_MS
...
internal suspend fun performFetch(context: Context, id: GlanceId) {
    ...
    try {
        ...
        com.meow.lnctattendance.database.OfflineCacheHelper(context).saveAttendance(
            data.present, data.absent, data.totalClasses, data.percentage, now
        )
        ...
        val cached = com.meow.lnctattendance.database.OfflineCacheHelper(context).getAttendance()
        if (cached != null) {
            updateAppWidgetState(context, id) { state ->
                state[KEY_PRESENT]    = cached.present
                state[KEY_ABSENT]     = cached.absent
                state[KEY_TOTAL]      = cached.total
                state[KEY_PERCENTAGE] = cached.percentage
                state[KEY_LAST_FETCH] = cached.lastFetch
